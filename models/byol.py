...
Credit:
This module adapted https://github.com/lucidrains/byol-pytorch 
"""
...
class BYOL(nn.Module):
    def __init__(
        ...
        # send mock time series tensors to instantiate singleton parameters
        self.forward(
            (
                torch.randn(2, temp_lenth, in_channels, device=device),
                torch.randn(2, temp_lenth, in_channels, device=device)
            )
        )
    ...
    def forward(self, input, return_embedding=False, return_projection=True):
        assert not (
            self.training and input[0][0].shape[0] == 1
        ...
        if return_embedding:
            return self.online_encoder(
                px_1, return_projection=return_projection
            ), self.online_encoder(px_2, return_projection=return_projection)

        online_proj_one, _ = self.online_encoder(px_1)
        online_proj_two, _ = self.online_encoder(px_2)
        ...
        with torch.no_grad():
            ...
            target_proj_one, _ = target_encoder(px_1)
            target_proj_two, _ = target_encoder(px_2)
