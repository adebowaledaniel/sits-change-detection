...
class TempCNN(torch.nn.Module):
    def __init__(self, input_dim=13, num_classes=19, sequencelength=45, kernel_size=7, hidden_dims=32, dropout=0.18203942949809093):
        super(TempCNN, self).__init__()
        self.modelname = f"TempCNN_input-dim={input_dim}_num-classes={num_classes}_sequencelenght={sequencelength}_" \
                         f"kernelsize={kernel_size}_hidden-dims={hidden_dims}"
        ...
        self.logit = nn.Linear(4 * hidden_dims, num_classes)
    ...
    def param_ratio(self):
        ...
        t = get_ntrainparams(self.conv_bn_relu1) + get_ntrainparams(self.conv_bn_relu2) + get_ntrainparams(self.conv_bn_relu3)
        c = get_ntrainparams(self.flatten) + get_ntrainparams(self.dense) + get_ntrainparams(self.logit)
